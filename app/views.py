...
class CutView(View):
    ...
    <ul>
        <li>Facet Alignment: Examine the precision with which the facets meet, especially at the culet (the point at the bottom of the <b>gem</b>). Ensure there is no culet facet (flat point at the bottom instead of a point), which is sometimes done to save weight but detracts from brilliance. **A sharp, well-defined point is ideal, unless a specific type of cut requires a different configuration**.</li>
        <li>Symmetry: For round brilliant cuts, when viewed from above, you should see two equally sized squares that are slightly offset from each other. When inspecting other shapes, look for consistent symmetry throughout the <b>gemstone</b>. </li>
        <li>Girdle: The girdle, the narrow band around the circumference of the gem, should be neither too thin nor too thick. An overly thin girdle can make the stone prone to chipping, while an overly thick girdle adds unnecessary weight without enhancing the appearance.</li>
        <li>Visual Size and Cut Proportions: A deeper cut will make a gem appear smaller face-up, while a shallower cut will make it seem larger for the same carat weight. Optimal cut proportions are crucial for maximising brilliance and perceived size.</li>
        <li>Windowing: A "window" is when you look at the center of the gem and see a hollow area, rather than light reflecting back from within. This occurs when light passes straight through the gem instead of being reflected back to the eye. A well-cut gem should not have a visible window, ensuring maximum brilliance and light return.</li>
       <li>Ideal Height Proportions: The height of the gem should ideally be approximately 70% of its diameter or width. These proportions are generally considered ideal for optimum light performance.</li>
        <li>Chips and Damage: Look for any chips, especially those that reach the surface (especially on the face-up side of the gem). Surface-reaching chips can dramatically decrease a gem's value and durability.</li>
    </ul>
